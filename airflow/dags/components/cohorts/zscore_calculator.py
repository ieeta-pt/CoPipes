...
def get_initial_variables(patientID):
    try:
        gender = SAHGlobalVariables.gender[patientID]
...
def calculate_zcore_tmta(patientID, measure):
    gender, eduy, age = get_initial_variables(patientID)
    if gender == FEMALE:
        if eduy < 13:
            if age > 79.99:
...
def calculate_zcore_animals_fluency(patient_id, measure):
    gender, eduy, age = get_initial_variables(patient_id)
    if eduy is None or age is None:
        return None
...
def calculate_zcore_avlt_delayed(patientID, measure):
    gender, eduy, age = get_initial_variables(patientID)
    if eduy is None or age is None or gender not in [MALE, FEMALE]:
        return None
    edulow = 1 if eduy < 7 else 0
    eduhigh = 1 if eduy > 12 else 0
...
def calculate_zcore_avlt_immediate(patientID, measure):
    ...
    eduhigh = 1 if eduy > 12 else 0
    sex = 1 if gender == MALE else 0
    return (measure - (49.672 + (-0.247 * (age - 50)) + (-0.0033 * (age - 50) ** 2) + (-4.227 * sex) + (-3.055 * edulow) + (2.496 * eduhigh))) / 7.826
...
def calculate_zcore_stroop_part1(patient_id, measure):
    ...
    return ((1 / measure) - (0.01566 + 0.000315 * age - 0.00112 * edulow + 0.001465 * eduhigh - 0.0000032 * age ** 2)) / 0.0034
