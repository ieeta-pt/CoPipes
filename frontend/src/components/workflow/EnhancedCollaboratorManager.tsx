...
interface EnhancedCollaboratorManagerProps {
  workflowName: string;
...
export default function EnhancedCollaboratorManager({
  workflowName,
  collaborators,
  userRole,
  canManagePermissions,
  onCollaboratorsChange,
}: EnhancedCollaboratorManagerProps) {
  const { token } = useAuth();
  ...
  const addCollaborator = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/workflows/${workflowName.replace(/ /g, "_")}/collaborators/enhanced`, {
        method: "POST",
        headers: {
  ...
  const updateCollaboratorRole = async (email: string, newRole: WorkflowRole) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/workflows/${workflowName.replace(/ /g, "_")}/collaborators/${encodeURIComponent(email)}/role`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || "Failed to update collaborator role");
      }
  ...
  const removeCollaborator = async (email: string) => {
    ...
    try {
      const response = await fetch(
        `/api/workflows/${workflowName.replace(/ /g, "_")}/collaborators/enhanced/${encodeURIComponent(email)}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${token}`,
  ...
  const fetchCollaborators = async () => {
    ...
    try {
      const response = await fetch(
        `/api/workflows/${workflowName.replace(/ /g, "_")}/collaborators/enhanced`,
        {
          headers: {
            "Authorization": `Bearer ${token}`,
          },
