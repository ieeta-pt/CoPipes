...
export default function AvatarStack({ 
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        setUsers(collaborators.map(email => ({ 
          email, 
          avatar_url: null, 
  ...
  if (!collaborators.length) {
    return <span className="text-base-content/50 text-sm">No collaborators</span>;
  }

  if (loading) {
    return (
      <div className="flex items-center">
        <div className={`${sizeClasses[size]} bg-base-200 rounded-full animate-pulse`}></div>
      </div>
    );
  }
    ...
    <div className="flex items-center">
            ...
            {getAvatarSrc(user) ? (
              <Image
                ...
                onError={(e) => {
                  ...
                  if (parent) {
                    parent.innerHTML = `<div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-400 to-purple-500 text-white font-medium">${getInitials(user.full_name || user.email)}</div>`;
                  }
                }}
              />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-400 to-purple-500 text-white font-medium">
      ...
      {users.length === 1 && (
        <span className="ml-2 text-sm text-base-content/70">
          {visibleUsers[0].full_name?.split(' ')[0] || visibleUsers[0].email.split('@')[0]}
        </span>
      )}
    </div>
  );
